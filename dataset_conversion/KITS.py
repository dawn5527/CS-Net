...
if __name__ == "__main__":
    ...
    def load_save_train(args):
        ...
        # kits
        img_npy = np.transpose(img_npy, [2, 1, 0])
        img_npy[img_npy < -300] = -300
        img_npy[img_npy > 300] = 300
        img_new = sitk.GetImageFromArray(img_npy)
        img_new.SetOrigin(img_itk.GetOrigin())
        img_new.SetDirection(img_itk.GetDirection())
        img_new.SetSpacing([img_itk.GetSpacing()[2],img_itk.GetSpacing()[1],img_itk.GetSpacing()[0]])
        sitk.WriteImage(img_new, join(img_dir, pat_id + "_0000.nii.gz"))
        ...
        img_new.SetDirection(img_itk.GetDirection())
        img_new.SetSpacing([img_itk.GetSpacing()[2], img_itk.GetSpacing()[1], img_itk.GetSpacing()[0]])
        sitk.WriteImage(img_new, join(lab_dir, pat_id + ".nii.gz"))
        return pat_id
    ...
    def load_save_test(args):
        ...
        img_new.SetDirection(img_itk.GetDirection())
        img_new.SetSpacing([img_itk.GetSpacing()[2],img_itk.GetSpacing()[1],img_itk.GetSpacing()[0]])
        sitk.WriteImage(img_new, join(img_dir_te, pat_id + "_0000.nii.gz"))
        return pat_id

    nii_files_tr_data = []
    nii_files_tr_seg = []
    patients = os.listdir(train_dir)
    patients = [patient for patient in patients if int(patient.split('_')[-1]) < 210]
    print(patients)
    for patient in patients:
        nii_files_tr_data.append(os.path.join(train_dir, patient, 'imaging.nii.gz'))
        nii_files_tr_seg.append(os.path.join(train_dir, patient, 'segmentation.nii.gz'))
    ...
    print(patients)
    for patient in patients:
        nii_files_ts.append(os.path.join(test_dir, patient, 'imaging.nii.gz'))
