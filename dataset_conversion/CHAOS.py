...
if __name__ == "__main__":
    ...
    def load_save_train(args):
        ...
        # print(pat_id)
        # print(pat_id.split("_")[0])
        pat_id = "train_" + pat_id

        # img_itk = sitk.ReadImage(data_file)
        reader = sitk.ImageSeriesReader()
        dicom_names = reader.GetGDCMSeriesFileNames(data_file)
        reader.SetFileNames(dicom_names)
        img_itk = reader.Execute()
        volume = sitk.GetArrayFromImage(img_itk)
        volume[volume < -200] = -200
        volume[volume > 250] = 250
        volume = sitk.GetImageFromArray(volume)
        volume.SetOrigin(img_itk.GetOrigin())
        volume.SetDirection(img_itk.GetDirection())
        volume.SetSpacing(img_itk.GetSpacing())
        sitk.WriteImage(volume, join(img_dir, pat_id + "_0000.nii.gz"))
    ...
    def load_save_test(args):
        ...
        volume.SetDirection(img_itk.GetDirection())
        volume.SetSpacing(img_itk.GetSpacing())
        sitk.WriteImage(volume, join(img_dir_te, pat_id + "_0000.nii.gz"))
        return pat_id

    nii_files_tr_data = []
    nii_files_tr_seg = []
    for patient in os.listdir(train_dir):
        nii_files_tr_data.append(os.path.join(train_dir,patient,'DICOM_anon'))
        nii_files_tr_seg.append(os.path.join(train_dir,patient,patient+'-seg.nii'))

    nii_files_ts = []
    for patient in os.listdir(test_dir):
        nii_files_ts.append(os.path.join(test_dir,patient,'DICOM_anon'))
