...
if __name__ == "__main__":
    ...
    for t in tasks:
        print(t)
        print(tasks)
        json_files_task = [i for i in subfiles(out_dir_all_json, prefix="Task%02.0d_" % int(t)) if i.find("ensemble") == -1]
        if len(json_files_task) > 0:
            ...
            for i in range(len(json_files_task) - 1):
                for j in range(i+1, len(json_files_task)):
                    ...
                    p = call(["python", join('/home/dawn/nnunet', "evaluation/model_selection/ensemble.py"), network1_folder, network2_folder, output_folder, task_name])
    ...
    with open(join(network_training_output_dir, "use_this_for_test.csv"), 'w') as f:
        for t in tasks:
            json_files_task = subfiles(out_dir_all_json, prefix="Task%02.0d_" % int(t))
            print(out_dir_all_json)
            print(json_files_task)
            if len(json_files_task) > 0:
                task_name = json_files_task[0].split("/")[-1].split("__")[0]
                print(task_name)
                mean_dice = []
