...
default_3D_augmentation_params = {
    ...
    "p_rot": 0.2,
    "dummy_2D": False,
    "mask_was_used_for_normalization": (0,False),
...
if __name__ == "__main__":
    from training.dataloading.dataset_loading import DataLoader2D, DataLoader3D, load_dataset
    from paths import preprocessing_output_dir
    import os
    import pickle

    t = "Task29_LITS"
    p = os.path.join(preprocessing_output_dir, t, "nnUNet_2D_stage0")
    dataset = load_dataset(p)
    with open(os.path.join(os.path.join(preprocessing_output_dir, t), "nnUNetPlans_plans_2D.pkl"), 'rb') as f:
        plans = pickle.load(f)
    # unpack_dataset(p)

    basic_patch_size = get_patch_size((256, 256),
    ...
    dl = DataLoader2D(dataset, (256, 256), (256, 256), 12, oversample_foreground_percent=0.33)
    tr, val = get_default_augmentation(dl, dl, (256, 256))
    n =1
    if n==1:
        data_dict = next(tr)
        data = data_dict['data']
        target = data_dict['target']
        np.savez('/data2/dawn/test', images=data)
        np.savez('/data2/dawn/test2', images=target)
        print('******************')
        print(target.shape)
        print('******************')
        n=2
