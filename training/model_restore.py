...
def restore_model(pkl_file, checkpoint=None, train=False):
    ...
    init = info['init']
    name = info['name']
    search_in = join("/home/dawn/nnunet", "training", "network_training")
    tr = recursive_find_trainer([search_in], name, current_module="training.network_training")
    if tr is None:
...
def load_model_and_checkpoint_files(folder, folds=None):
    ...
    if torch.cuda.is_available():
        ...
        map_location = 'cpu'
    # dawn changed here
    all_params = [torch.load(i, map_location=map_location) for i in all_best_model_files]
