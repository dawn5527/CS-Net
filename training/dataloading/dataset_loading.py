...
from batchgenerators.augmentations.utils import random_crop_2D_image_batched, pad_nd_image, random_crop_3D_image_batched
import numpy as np
...
def crop_2D_image_force_fg(img, crop_size, force_class=None):
    ...
    return result
def crop_3D_image_force_fg(img, crop_size, force_class=None):
    """
    img must be [c, x, y]
    img[-1] must be the segmentation with segmentation>0 being foreground
    :param img:
    ...
    # we need to find the center coords that we can crop to without exceeding the image border
    lb_x = crop_size[0] // 2
    ub_x = img.shape[2] - crop_size[0] // 2 - crop_size[0] % 2
    lb_y = crop_size[1] // 2
    ub_y = img.shape[3] - crop_size[1] // 2 - crop_size[1] % 2
    ...
    for i in range(2):
        selected_center_voxel[i] = max(crop_size[i]//2, selected_center_voxel[i])
        selected_center_voxel[i] = min(img.shape[i+2] - crop_size[i]//2 - crop_size[i] % 2, selected_center_voxel[i])
...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                random_slice = np.random.randint(1, case_all_data.shape[leading_axis + 1])
            else:
            ...
            if self.pseudo_3d_slices == 1:
                if leading_axis == 0:
                    case_all_data = case_all_data[:, random_slice-1:random_slice+2]
                elif leading_axis == 1:
            ...
            if np.any(self.need_to_pad) > 0:
                self.need_to_pad = np.array([0, self.need_to_pad[0], self.need_to_pad[1]])
                new_shp = np.array(case_all_data.shape[1:] + self.need_to_pad)
                if np.any(new_shp - np.array([3, self.patch_size[0], self.patch_size[1]]) < 0):
                    new_shp = np.max(
                        np.vstack((new_shp[None], np.array([3, self.patch_size[0], self.patch_size[1]])[None])), 0)
            else:
                if np.any(
                        np.array(case_all_data.shape[1:]) - np.array([3, self.patch_size[0], self.patch_size[1]]) < 0):
                    new_shp = np.max(
                        np.vstack((np.array(case_all_data.shape[1:])[None],
                                   np.array([3, self.patch_size[0], self.patch_size[1]])[None])), 0)
            ...
            selected_class = 1
            if not force_fg:
                case_all_data = random_crop_3D_image_batched(case_all_data, tuple([3, self.patch_size[0], self.patch_size[1]]))
            else:
                case_all_data = crop_3D_image_force_fg(case_all_data[0], tuple(self.patch_size), selected_class)[None]
...
if __name__ == "__main__":
    t = "Task32_KITS"
    p = join(preprocessing_output_dir, t, "nnUNet_2D_stage0")
    dataset = load_dataset(p)
    with open(join(join(preprocessing_output_dir, t), "nnUNetPlans_plans_2D.pkl"), 'rb') as f:
        plans = pickle.load(f)
    unpack_dataset(p)
    # dl = DataLoader3D(dataset, (3, 256, 256), (3, 256, 256), 2, oversample_foreground_percent=0.33)
    # dl_tr = dl.generate_train_batch()
    ...
    dl2d = DataLoader2D(dataset, (256, 256), (256, 256), 12, oversample_foreground_percent=0.67)
    dl_tr = dl2d.generate_train_batch()
